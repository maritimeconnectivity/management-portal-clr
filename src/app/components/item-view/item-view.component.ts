...
import {ItemType, itemTypeToString, MCPComponentContext} from 'src/app/common/menuType';
...
export class ItemViewComponent implements OnChanges {
    ...
    ngOnChanges(simpleChange: any) {
        ...
        if (this.itemType === ItemType.Organization && !this.hasEditPermission) {
            this.itemManagerService.fetchMyRolesInOrg(this.orgMrn).then((roles) => {
                this.hasEditPermission = this.authService.hasPermission(ItemType.Organization, roles, MCPComponentContext.MIR, this.orgMrn === this.itemId);
            })
        }
